         ...
         <div class="row">
            ...
            <div class="col-md-3">
               ...
               <div class="mb-3 {{ $method == 'order' ? '' : 'd-none' }}" id="orderAmountDiv">
                  ...
                  <input type="number" name="byOrder" id="order" class="form-control" value="{{ $orderAmount }}" placeholder="Enter Amount" {{ $method == 'order' ? 'required' : '' }}>
               </div>
               <!-- Contract Number Input -->
               <div class="mb-3 {{ $method == 'contract' ? '' : 'd-none' }}" id="contractNumberDiv">
                  <label class="form-label">📄 Contract Number</label>
                  <input type="text" name="byContract" id="contract" class="form-control" value="{{ $contractNumber }}" placeholder="Enter Contract Number" {{ $method == 'contract' ? 'required' : '' }}>
               </div>
               <div class="mb-3">
                
                <input type="text" id="totalAmountInput" class="form-control" value="{{ $orderAmount }}"value="{{ $contractNumber }}" readonly>
            </div>
         ...
         <div class="row mt-4" >
               ...
               <div class="col-md-4">
                  <div class="mb-3">
                     ...
                     <select name="lr[{{ $index }}][delivery_mode]" class="form-select" required>
                        ...
                        <option value="door_delivery" {{ old("lr.$index.delivery_mode", $lr['delivery_mode']) == 'door_delivery' ? 'selected' : '' }}>Door Deliver</option>
                        <option value="godwon_deliver" {{ old("lr.$index.delivery_mode", $lr['delivery_mode']) == 'godwon_deliver' ? 'selected' : '' }}>Godwon Deliver</option>
                     </select>
                  </div>
               </div>
               <!-- From Location -->
            ...
            <div class="row mt-4">
                        ...
                        <tbody id="cargoTableBody-{{ $index }}">
                           ...
                           <tr>
                              ...
                              <td><input type="number" 
                                 name="lr[{{ $index }}][cargo][{{ $cargoIndex }}][charged_weight]" 
                                 class="form-control charged-weight" 
                                 value="{{ $cargo['charged_weight'] }}" 
                                 required 
                              ...
                              <td>
                                 <input  name="lr[{{ $index }}][cargo][{{ $cargoIndex }}][declared_value]"
                                    ...
                                    placeholder="0" 
                                    required 
                                    oninput="calculateTotalDeclaredValue({{ $index }})">
                              </td>
                              <td>
               ...
               <div class="row mt-3">
                  <div class="col-md-6">
                     <div class="mt-3">
                        <label><strong>⚖️ Total Charged Weight</strong></label>
                        <input type="text" 
                           id="totalChargedWeight-{{ $index }}" 
                           name="lr[{{ $index }}][total_charged_weight]" 
                           class="form-control" 
                           value="" 
                           readonly>
                        
                        <!-- Total Declared Value Display (Existing) -->
                        <label><strong>💰 Total Declared Value (Rs.)</strong></label>
                        <input type="text" 
                           id="totalDeclaredValue-{{ $index }}" 
                           name="lr[{{ $index }}][total_declared_value]" 
                           class="form-control" 
                           value="" 
                           readonly>
...
<script>
   ...
   // Show alert for contract amount
   function showContractAmountAlert(value) {
       if (value) {
           currentContractAmount = parseFloat(value) || 0;
           document.getElementById('contractAmountInput').value = currentContractAmount;
           updateFinalResult();
       }
   }
   
   // Function to update final result based on charged weight and either order amount or contract amount
   function updateFinalResult() {
       // Get total charged weight
       let chargedWeight = parseFloat(document.getElementById(`totalChargedWeight-${lrIndex}`).value) || 0;
       
       // Use either orderAmount or contractAmount based on available value
       let finalAmount = 0;
       if (currentOrderAmount > 0) {
           finalAmount = chargedWeight * currentOrderAmount;
       } else if (currentContractAmount > 0) {
           finalAmount = chargedWeight * currentContractAmount;
       }
   ...
   function calculateTotalDeclaredValue(lrIndex) {
       ...
       // Specific LR section ke andar hi search karenge
       const lrSection = document.querySelector(`#lr_section_${lrIndex}`);
       if (!lrSection) return; // Agar nahi mila to kuch na kare
   
       lrSection.querySelectorAll(`input[name^="lr[${lrIndex}][cargo]"][name$="[declared_value]"]`).forEach(input => {
       ...
       // Update the total declared value field
       const totalInput = document.querySelector(`#total_declared_value_${lrIndex}`);
       if (totalInput) {
   ...
   function calculateTotalChargedWeight(lrIndex) {
       ...
       lrSection.querySelectorAll(`input[name^="lr[${lrIndex}][cargo]"][name$="[charged_weight]"]`).forEach(input => {
           const value = parseFloat(input.value);
           if (!isNaN(value)) {
               total += value;
           }
       ...
       // Update the total charged weight field
       const totalInput = document.querySelector(`#total_charged_weight_${lrIndex}`);
       if (totalInput) {
           totalInput.value = total;
       }
   }
